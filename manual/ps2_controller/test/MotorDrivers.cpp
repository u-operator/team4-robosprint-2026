...
void A4988_Stepper::update() {
  if (_stepsRemaining > 0) {
    unsigned long currentMicros = micros();
    if (currentMicros - _lastStepTime >= _stepDelay) {
      _lastStepTime = currentMicros;
      if (_pulseState == LOW) {
        digitalWrite(_step, HIGH);   // A4988: STEP rising edge triggers a step
        delayMicroseconds(2);        // A4988 requires >= 1µs STEP pulse width
        digitalWrite(_step, LOW);
        _pulseState = HIGH;          // Mark that the step was fired
        _stepsRemaining--;           // Decrement after each complete step pulse
      } else {
        _pulseState = LOW;           // Ready for next step
      }
    }
  }
}